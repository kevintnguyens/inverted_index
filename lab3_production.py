...
def searchIndexUI():
    ...
    *** Returns a search query string  
    ...
    while(query is not "q"): #since query is an empty string aka not q, the loop will run atleast once
        ...
        print(query)
        if query is not "q": #if the user doesn't enter "q"
            results = searchIndex(invertedIndex, comboDict, query,5) #stores the five URLs computed from the searchIndex()into results
            count=0 
            for line in results:
                count+=1
